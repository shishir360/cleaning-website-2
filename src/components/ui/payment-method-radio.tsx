...
export const radios: PaymentOption[] = [
  {
    name: "Credit Card",
    description: "Secure, encrypted credit card processing.",
    icon: (
      <svg
        width="24"
        height="24"
      ...
      >
        <path d="M15.2436 6.17905H8.75391V17.8412H15.2436V6.17905Z" fill="#c0a062" />
        <path
          ...
          return (
            <li key={id}>
              <label htmlFor={id} className="block relative">
                ...
                <div 
                  className={`w-full flex gap-x-3 items-start p-4 cursor-pointer rounded-2xl border transition-all duration-300 shadow-sm ${
                    isSelected 
                    ? "bg-primary/[0.02] border-tertiary ring-1 ring-tertiary ring-offset-0" 
                    : "bg-inverted border-black/10 hover:border-black/20"
                  }`}
                >
                  <div className="flex-none bg-paper w-12 h-12 flex items-center justify-center rounded-xl">{item.icon}</div>
                  <div className="pt-1">
                    <h3 className={`leading-none font-medium pr-3 ${isSelected ? "text-primary" : "text-secondary"}`}>
                      {item.name}
                    </h3>
                    <p className="mt-1.5 text-sm font-light text-secondary/80">{item.description}</p>
                  </div>
                </div>

                {/* Checkmark */}
                <div className={`absolute top-5 right-5 flex items-center justify-center w-5 h-5 rounded-full border transition-all duration-300 ${
                  isSelected 
